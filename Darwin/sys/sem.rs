//! This file has been automatically generated by `objc2`'s `header-translator`.
//! DO NOT EDIT
use core::ffi::*;
#[cfg(feature = "objc2")]
use objc2::__framework_prelude::*;

use crate::ffi::*;

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/__semid_ds_new?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct __semid_ds_new {
    pub sem_perm: libc::ipc_perm,
    pub sem_base: i32,
    pub sem_nsems: c_ushort,
    pub sem_otime: libc::time_t,
    pub sem_pad1: i32,
    pub sem_ctime: libc::time_t,
    pub sem_pad2: i32,
    pub sem_pad3: [i32; 4],
}

unsafe impl Encode for __semid_ds_new {
    const ENCODING: Encoding = Encoding::Struct(
        "__semid_ds_new",
        &[
            <libc::ipc_perm>::ENCODING,
            <i32>::ENCODING,
            <c_ushort>::ENCODING,
            <libc::time_t>::ENCODING,
            <i32>::ENCODING,
            <libc::time_t>::ENCODING,
            <i32>::ENCODING,
            <[i32; 4]>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for __semid_ds_new {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/sem?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct sem {
    pub semval: c_ushort,
    pub sempid: libc::pid_t,
    pub semncnt: c_ushort,
    pub semzcnt: c_ushort,
}

unsafe impl Encode for sem {
    const ENCODING: Encoding = Encoding::Struct(
        "sem",
        &[
            <c_ushort>::ENCODING,
            <libc::pid_t>::ENCODING,
            <c_ushort>::ENCODING,
            <c_ushort>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for sem {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/sembuf?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct sembuf {
    pub sem_num: c_ushort,
    pub sem_op: c_short,
    pub sem_flg: c_short,
}

unsafe impl Encode for sembuf {
    const ENCODING: Encoding = Encoding::Struct(
        "sembuf",
        &[
            <c_ushort>::ENCODING,
            <c_short>::ENCODING,
            <c_short>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for sembuf {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/semun?language=objc)
#[repr(C)]
#[derive(Clone, Copy)]
pub union semun {
    pub val: c_int,
    pub buf: *mut libc::__semid_ds_new,
    pub array: *mut c_ushort,
}

unsafe impl Encode for semun {
    const ENCODING: Encoding = Encoding::Union(
        "semun",
        &[
            <c_int>::ENCODING,
            <*mut libc::__semid_ds_new>::ENCODING,
            <*mut c_ushort>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for semun {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/semun_t?language=objc)
pub type semun_t = libc::semun;

extern "C-unwind" {
    pub fn semget(param1: libc::key_t, param1: c_int, param1: c_int) -> c_int;
}

extern "C-unwind" {
    pub fn semop(param1: c_int, param1: *mut libc::sembuf, param1: usize) -> c_int;
}
