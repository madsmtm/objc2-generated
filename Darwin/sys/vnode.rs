//! This file has been automatically generated by `objc2`'s `header-translator`.
//! DO NOT EDIT
use core::ffi::*;
#[cfg(feature = "objc2")]
use objc2::__framework_prelude::*;

use crate::ffi::*;

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/vtype?language=objc)
#[repr(transparent)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct vtype(pub c_uint);
impl vtype {
    pub const VNON: Self = Self(0);
    pub const VREG: Self = Self(1);
    pub const VDIR: Self = Self(2);
    pub const VBLK: Self = Self(3);
    pub const VCHR: Self = Self(4);
    pub const VLNK: Self = Self(5);
    pub const VSOCK: Self = Self(6);
    pub const VFIFO: Self = Self(7);
    pub const VBAD: Self = Self(8);
    pub const VSTR: Self = Self(9);
    pub const VCPLX: Self = Self(10);
}

unsafe impl Encode for vtype {
    const ENCODING: Encoding = c_uint::ENCODING;
}

unsafe impl RefEncode for vtype {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/vtagtype?language=objc)
#[repr(transparent)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct vtagtype(pub c_uint);
impl vtagtype {
    pub const VT_NON: Self = Self(0);
    pub const VT_UFS: Self = Self(1);
    pub const VT_NFS: Self = Self(2);
    pub const VT_MFS: Self = Self(3);
    pub const VT_MSDOSFS: Self = Self(4);
    pub const VT_LFS: Self = Self(5);
    pub const VT_LOFS: Self = Self(6);
    pub const VT_FDESC: Self = Self(7);
    pub const VT_PORTAL: Self = Self(8);
    pub const VT_NULL: Self = Self(9);
    pub const VT_UMAP: Self = Self(10);
    pub const VT_KERNFS: Self = Self(11);
    pub const VT_PROCFS: Self = Self(12);
    pub const VT_AFS: Self = Self(13);
    pub const VT_ISOFS: Self = Self(14);
    pub const VT_MOCKFS: Self = Self(15);
    pub const VT_HFS: Self = Self(16);
    pub const VT_ZFS: Self = Self(17);
    pub const VT_DEVFS: Self = Self(18);
    pub const VT_WEBDAV: Self = Self(19);
    pub const VT_UDF: Self = Self(20);
    pub const VT_AFP: Self = Self(21);
    pub const VT_CDDA: Self = Self(22);
    pub const VT_CIFS: Self = Self(23);
    pub const VT_OTHER: Self = Self(24);
    pub const VT_APFS: Self = Self(25);
    pub const VT_LOCKERFS: Self = Self(26);
    pub const VT_BINDFS: Self = Self(27);
}

unsafe impl Encode for vtagtype {
    const ENCODING: Encoding = c_uint::ENCODING;
}

unsafe impl RefEncode for vtagtype {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/io_compression_stats?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct io_compression_stats {
    pub uncompressed_size: u64,
    pub compressed_size: u64,
    pub buffer_size_compression_dist: [ArrayUnknownABI<[u32; 7]>; 10],
    pub block_compressed_size_dist: [u32; 16],
}

unsafe impl Encode for io_compression_stats {
    const ENCODING: Encoding = Encoding::Struct(
        "io_compression_stats",
        &[
            <u64>::ENCODING,
            <u64>::ENCODING,
            <[ArrayUnknownABI<[u32; 7]>; 10]>::ENCODING,
            <[u32; 16]>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for io_compression_stats {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/io_compression_stats_t?language=objc)
pub type io_compression_stats_t = *mut libc::io_compression_stats;

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/iocs_store_buffer_entry?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct iocs_store_buffer_entry {
    pub path_name: [c_char; 128],
    pub iocs: libc::io_compression_stats,
}

unsafe impl Encode for iocs_store_buffer_entry {
    const ENCODING: Encoding = Encoding::Struct(
        "iocs_store_buffer_entry",
        &[
            <[c_char; 128]>::ENCODING,
            <libc::io_compression_stats>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for iocs_store_buffer_entry {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}
