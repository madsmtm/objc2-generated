//! This file has been automatically generated by `objc2`'s `header-translator`.
//! DO NOT EDIT
use core::ffi::*;
#[cfg(feature = "objc2")]
use objc2::__framework_prelude::*;

use crate::ffi::*;

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/tsegqe_head?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct tsegqe_head {
    pub lh_first: u32,
}

unsafe impl Encode for tsegqe_head {
    const ENCODING: Encoding = Encoding::Struct("tsegqe_head", &[<u32>::ENCODING]);
}

unsafe impl RefEncode for tsegqe_head {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/tcpcb?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct tcpcb {
    pub t_segq: libc::tsegqe_head,
    pub t_dupacks: c_int,
    pub unused: u32,
    pub t_timer: [c_int; 4],
    pub t_inpcb: u32,
    pub t_state: c_int,
    pub t_flags: c_uint,
    pub t_force: c_int,
    pub snd_una: libc::tcp_seq,
    pub snd_max: libc::tcp_seq,
    pub snd_nxt: libc::tcp_seq,
    pub snd_up: libc::tcp_seq,
    pub snd_wl1: libc::tcp_seq,
    pub snd_wl2: libc::tcp_seq,
    pub iss: libc::tcp_seq,
    pub irs: libc::tcp_seq,
    pub rcv_nxt: libc::tcp_seq,
    pub rcv_adv: libc::tcp_seq,
    pub rcv_wnd: u32,
    pub rcv_up: libc::tcp_seq,
    pub snd_wnd: u32,
    pub snd_cwnd: u32,
    pub snd_ssthresh: u32,
    pub t_maxopd: c_uint,
    pub t_rcvtime: u32,
    pub t_starttime: u32,
    pub t_rtttime: c_int,
    pub t_rtseq: libc::tcp_seq,
    pub t_rxtcur: c_int,
    pub t_maxseg: c_uint,
    pub t_srtt: c_int,
    pub t_rttvar: c_int,
    pub t_rxtshift: c_int,
    pub t_rttmin: c_uint,
    pub t_rttupdated: u32,
    pub max_sndwnd: u32,
    pub t_softerror: c_int,
    pub t_oobflags: c_char,
    pub t_iobc: c_char,
    pub snd_scale: c_uchar,
    pub rcv_scale: c_uchar,
    pub request_r_scale: c_uchar,
    pub requested_s_scale: c_uchar,
    pub ts_recent: u32,
    pub ts_recent_age: u32,
    pub last_ack_sent: libc::tcp_seq,
    pub cc_send: libc::tcp_cc,
    pub cc_recv: libc::tcp_cc,
    pub snd_recover: libc::tcp_seq,
    pub snd_cwnd_prev: u32,
    pub snd_ssthresh_prev: u32,
    pub t_badrxtwin: u32,
}

unsafe impl Encode for tcpcb {
    const ENCODING: Encoding = Encoding::Struct(
        "tcpcb",
        &[
            <libc::tsegqe_head>::ENCODING,
            <c_int>::ENCODING,
            <u32>::ENCODING,
            <[c_int; 4]>::ENCODING,
            <u32>::ENCODING,
            <c_int>::ENCODING,
            <c_uint>::ENCODING,
            <c_int>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <u32>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <c_uint>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <c_int>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <c_int>::ENCODING,
            <c_uint>::ENCODING,
            <c_int>::ENCODING,
            <c_int>::ENCODING,
            <c_int>::ENCODING,
            <c_uint>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <c_int>::ENCODING,
            <c_char>::ENCODING,
            <c_char>::ENCODING,
            <c_uchar>::ENCODING,
            <c_uchar>::ENCODING,
            <c_uchar>::ENCODING,
            <c_uchar>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_cc>::ENCODING,
            <libc::tcp_cc>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for tcpcb {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/tcpstat?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct tcpstat {
    pub tcps_connattempt: u32,
    pub tcps_accepts: u32,
    pub tcps_connects: u32,
    pub tcps_drops: u32,
    pub tcps_conndrops: u32,
    pub tcps_closed: u32,
    pub tcps_segstimed: u32,
    pub tcps_rttupdated: u32,
    pub tcps_delack: u32,
    pub tcps_timeoutdrop: u32,
    pub tcps_rexmttimeo: u32,
    pub tcps_persisttimeo: u32,
    pub tcps_keeptimeo: u32,
    pub tcps_keepprobe: u32,
    pub tcps_keepdrops: u32,
    pub tcps_sndtotal: u32,
    pub tcps_sndpack: u32,
    pub tcps_sndbyte: u32,
    pub tcps_sndrexmitpack: u32,
    pub tcps_sndrexmitbyte: u32,
    pub tcps_sndacks: u32,
    pub tcps_sndprobe: u32,
    pub tcps_sndurg: u32,
    pub tcps_sndwinup: u32,
    pub tcps_sndctrl: u32,
    pub tcps_rcvtotal: u32,
    pub tcps_rcvpack: u32,
    pub tcps_rcvbyte: u32,
    pub tcps_rcvbadsum: u32,
    pub tcps_rcvbadoff: u32,
    pub tcps_rcvmemdrop: u32,
    pub tcps_rcvshort: u32,
    pub tcps_rcvduppack: u32,
    pub tcps_rcvdupbyte: u32,
    pub tcps_rcvpartduppack: u32,
    pub tcps_rcvpartdupbyte: u32,
    pub tcps_rcvoopack: u32,
    pub tcps_rcvoobyte: u32,
    pub tcps_rcvpackafterwin: u32,
    pub tcps_rcvbyteafterwin: u32,
    pub tcps_rcvafterclose: u32,
    pub tcps_rcvwinprobe: u32,
    pub tcps_rcvdupack: u32,
    pub tcps_rcvacktoomuch: u32,
    pub tcps_rcvackpack: u32,
    pub tcps_rcvackbyte: u32,
    pub tcps_rcvwinupd: u32,
    pub tcps_pawsdrop: u32,
    pub tcps_predack: u32,
    pub tcps_preddat: u32,
    pub tcps_pcbcachemiss: u32,
    pub tcps_cachedrtt: u32,
    pub tcps_cachedrttvar: u32,
    pub tcps_cachedssthresh: u32,
    pub tcps_usedrtt: u32,
    pub tcps_usedrttvar: u32,
    pub tcps_usedssthresh: u32,
    pub tcps_persistdrop: u32,
    pub tcps_badsyn: u32,
    pub tcps_mturesent: u32,
    pub tcps_listendrop: u32,
    pub tcps_synchallenge: u32,
    pub tcps_rstchallenge: u32,
    pub tcps_minmssdrops: u32,
    pub tcps_sndrexmitbad: u32,
    pub tcps_badrst: u32,
    pub tcps_sc_added: u32,
    pub tcps_sc_retransmitted: u32,
    pub tcps_sc_dupsyn: u32,
    pub tcps_sc_dropped: u32,
    pub tcps_sc_completed: u32,
    pub tcps_sc_bucketoverflow: u32,
    pub tcps_sc_cacheoverflow: u32,
    pub tcps_sc_reset: u32,
    pub tcps_sc_stale: u32,
    pub tcps_sc_aborted: u32,
    pub tcps_sc_badack: u32,
    pub tcps_sc_unreach: u32,
    pub tcps_sc_zonefail: u32,
    pub tcps_sc_sendcookie: u32,
    pub tcps_sc_recvcookie: u32,
    pub tcps_hc_added: u32,
    pub tcps_hc_bucketoverflow: u32,
    pub tcps_sack_recovery_episode: u32,
    pub tcps_sack_rexmits: u32,
    pub tcps_sack_rexmit_bytes: u32,
    pub tcps_sack_rcv_blocks: u32,
    pub tcps_sack_send_blocks: u32,
    pub tcps_sack_sboverflow: u32,
    pub tcps_rack_recovery_episode: u32,
    pub tcps_rack_reordering_timeout_recovery_episode: u32,
    pub tcps_rack_rexmits: u32,
    pub tcps_bg_rcvtotal: u32,
    pub tcps_rxtfindrop: u32,
    pub tcps_fcholdpacket: u32,
    pub tcps_limited_txt: u32,
    pub tcps_early_rexmt: u32,
    pub tcps_sack_ackadv: u32,
    pub tcps_rcv_swcsum: u32,
    pub tcps_rcv_swcsum_bytes: u32,
    pub tcps_rcv6_swcsum: u32,
    pub tcps_rcv6_swcsum_bytes: u32,
    pub tcps_snd_swcsum: u32,
    pub tcps_snd_swcsum_bytes: u32,
    pub tcps_snd6_swcsum: u32,
    pub tcps_snd6_swcsum_bytes: u32,
    pub tcps_unused_1: u32,
    pub tcps_unused_2: u32,
    pub tcps_unused_3: u32,
    pub tcps_invalid_mpcap: u32,
    pub tcps_invalid_joins: u32,
    pub tcps_mpcap_fallback: u32,
    pub tcps_join_fallback: u32,
    pub tcps_estab_fallback: u32,
    pub tcps_invalid_opt: u32,
    pub tcps_mp_outofwin: u32,
    pub tcps_mp_reducedwin: u32,
    pub tcps_mp_badcsum: u32,
    pub tcps_mp_oodata: u32,
    pub tcps_mp_switches: u32,
    pub tcps_mp_rcvtotal: u32,
    pub tcps_mp_rcvbytes: u32,
    pub tcps_mp_sndpacks: u32,
    pub tcps_mp_sndbytes: u32,
    pub tcps_join_rxmts: u32,
    pub tcps_tailloss_rto: u32,
    pub tcps_reordered_pkts: u32,
    pub tcps_recovered_pkts: u32,
    pub tcps_pto: u32,
    pub tcps_rto_after_pto: u32,
    pub tcps_tlp_recovery: u32,
    pub tcps_tlp_recoverlastpkt: u32,
    pub tcps_ecn_client_success: u32,
    pub tcps_ecn_recv_ece: u32,
    pub tcps_ecn_sent_ece: u32,
    pub tcps_detect_reordering: u32,
    pub tcps_delay_recovery: u32,
    pub tcps_avoid_rxmt: u32,
    pub tcps_unnecessary_rxmt: u32,
    pub tcps_nostretchack: u32,
    pub tcps_rescue_rxmt: u32,
    pub tcps_pto_in_recovery: u32,
    pub tcps_pmtudbh_reverted: u32,
    pub tcps_dsack_disable: u32,
    pub tcps_dsack_ackloss: u32,
    pub tcps_dsack_badrexmt: u32,
    pub tcps_dsack_sent: u32,
    pub tcps_dsack_recvd: u32,
    pub tcps_dsack_recvd_old: u32,
    pub tcps_mp_sel_symtomsd: u32,
    pub tcps_mp_sel_rtt: u32,
    pub tcps_mp_sel_rto: u32,
    pub tcps_mp_sel_peer: u32,
    pub tcps_mp_num_probes: u32,
    pub tcps_mp_verdowngrade: u32,
    pub tcps_drop_after_sleep: u32,
    pub tcps_probe_if: u32,
    pub tcps_probe_if_conflict: u32,
    pub tcps_ecn_client_setup: u32,
    pub tcps_ecn_server_setup: u32,
    pub tcps_ecn_server_success: u32,
    pub tcps_ecn_ace_syn_not_ect: u32,
    pub tcps_ecn_ace_syn_ect1: u32,
    pub tcps_ecn_ace_syn_ect0: u32,
    pub tcps_ecn_ace_syn_ce: u32,
    pub tcps_ecn_lost_synack: u32,
    pub tcps_ecn_lost_syn: u32,
    pub tcps_ecn_not_supported: u32,
    pub tcps_ecn_recv_ce: u32,
    pub tcps_ecn_ace_recv_ce: u32,
    pub tcps_ecn_conn_recv_ce: u32,
    pub tcps_ecn_conn_recv_ece: u32,
    pub tcps_ecn_conn_plnoce: u32,
    pub tcps_ecn_conn_pl_ce: u32,
    pub tcps_ecn_conn_nopl_ce: u32,
    pub tcps_ecn_fallback_synloss: u32,
    pub tcps_ecn_fallback_reorder: u32,
    pub tcps_ecn_fallback_ce: u32,
    pub tcps_tfo_syn_data_rcv: u32,
    pub tcps_tfo_cookie_req_rcv: u32,
    pub tcps_tfo_cookie_sent: u32,
    pub tcps_tfo_cookie_invalid: u32,
    pub tcps_tfo_cookie_req: u32,
    pub tcps_tfo_cookie_rcv: u32,
    pub tcps_tfo_syn_data_sent: u32,
    pub tcps_tfo_syn_data_acked: u32,
    pub tcps_tfo_syn_loss: u32,
    pub tcps_tfo_blackhole: u32,
    pub tcps_tfo_cookie_wrong: u32,
    pub tcps_tfo_no_cookie_rcv: u32,
    pub tcps_tfo_heuristics_disable: u32,
    pub tcps_tfo_sndblackhole: u32,
    pub tcps_mss_to_default: u32,
    pub tcps_mss_to_medium: u32,
    pub tcps_mss_to_low: u32,
    pub tcps_ecn_fallback_droprst: u32,
    pub tcps_ecn_fallback_droprxmt: u32,
    pub tcps_ecn_fallback_synrst: u32,
    pub tcps_mptcp_rcvmemdrop: u32,
    pub tcps_mptcp_rcvduppack: u32,
    pub tcps_mptcp_rcvpackafterwin: u32,
    pub tcps_timer_drift_le_1_ms: u32,
    pub tcps_timer_drift_le_10_ms: u32,
    pub tcps_timer_drift_le_20_ms: u32,
    pub tcps_timer_drift_le_50_ms: u32,
    pub tcps_timer_drift_le_100_ms: u32,
    pub tcps_timer_drift_le_200_ms: u32,
    pub tcps_timer_drift_le_500_ms: u32,
    pub tcps_timer_drift_le_1000_ms: u32,
    pub tcps_timer_drift_gt_1000_ms: u32,
    pub tcps_mptcp_handover_attempt: u32,
    pub tcps_mptcp_interactive_attempt: u32,
    pub tcps_mptcp_aggregate_attempt: u32,
    pub tcps_mptcp_fp_handover_attempt: u32,
    pub tcps_mptcp_fp_interactive_attempt: u32,
    pub tcps_mptcp_fp_aggregate_attempt: u32,
    pub tcps_mptcp_heuristic_fallback: u32,
    pub tcps_mptcp_fp_heuristic_fallback: u32,
    pub tcps_mptcp_handover_success_wifi: u32,
    pub tcps_mptcp_handover_success_cell: u32,
    pub tcps_mptcp_interactive_success: u32,
    pub tcps_mptcp_aggregate_success: u32,
    pub tcps_mptcp_fp_handover_success_wifi: u32,
    pub tcps_mptcp_fp_handover_success_cell: u32,
    pub tcps_mptcp_fp_interactive_success: u32,
    pub tcps_mptcp_fp_aggregate_success: u32,
    pub tcps_mptcp_handover_cell_from_wifi: u32,
    pub tcps_mptcp_handover_wifi_from_cell: u32,
    pub tcps_mptcp_interactive_cell_from_wifi: u32,
    pub tcps_mptcp_handover_cell_bytes: u64,
    pub tcps_mptcp_interactive_cell_bytes: u64,
    pub tcps_mptcp_aggregate_cell_bytes: u64,
    pub tcps_mptcp_handover_all_bytes: u64,
    pub tcps_mptcp_interactive_all_bytes: u64,
    pub tcps_mptcp_aggregate_all_bytes: u64,
    pub tcps_mptcp_back_to_wifi: u32,
    pub tcps_mptcp_wifi_proxy: u32,
    pub tcps_mptcp_cell_proxy: u32,
    pub tcps_ka_offload_drops: u32,
    pub tcps_mptcp_triggered_cell: u32,
    pub tcps_fin_timeout_drops: u32,
}

unsafe impl Encode for tcpstat {
    const ENCODING: Encoding = Encoding::Struct(
        "tcpstat",
        &[
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for tcpstat {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/tcpstat_local?language=objc)
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct tcpstat_local {
    pub badformat: u64,
    pub unspecv6: u64,
    pub synfin: u64,
    pub badformatipsec: u64,
    pub noconnnolist: u64,
    pub noconnlist: u64,
    pub listbadsyn: u64,
    pub icmp6unreach: u64,
    pub deprecate6: u64,
    pub ooopacket: u64,
    pub rstinsynrcv: u64,
    pub dospacket: u64,
    pub cleanup: u64,
    pub synwindow: u64,
}

unsafe impl Encode for tcpstat_local {
    const ENCODING: Encoding = Encoding::Struct(
        "tcpstat_local",
        &[
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
            <u64>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for tcpstat_local {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/xtcpcb?language=objc)
#[repr(C)]
#[derive(Clone, Copy)]
pub struct xtcpcb {
    pub xt_len: u32,
    pub xt_inp: libc::inpcb,
    pub xt_tp: libc::tcpcb,
    pub xt_socket: libc::xsocket,
    pub xt_alignment_hack: libc::u_quad_t,
}

unsafe impl Encode for xtcpcb {
    const ENCODING: Encoding = Encoding::Struct(
        "xtcpcb",
        &[
            <u32>::ENCODING,
            <libc::inpcb>::ENCODING,
            <libc::tcpcb>::ENCODING,
            <libc::xsocket>::ENCODING,
            <libc::u_quad_t>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for xtcpcb {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}

/// [Apple's documentation](https://developer.apple.com/documentation/darwin/xtcpcb64?language=objc)
#[repr(C)]
#[derive(Clone, Copy)]
pub struct xtcpcb64 {
    pub xt_len: u32,
    pub xt_inpcb: libc::xinpcb64,
    pub t_segq: u64,
    pub t_dupacks: c_int,
    pub t_timer: [c_int; 4],
    pub t_state: c_int,
    pub t_flags: c_uint,
    pub t_force: c_int,
    pub snd_una: libc::tcp_seq,
    pub snd_max: libc::tcp_seq,
    pub snd_nxt: libc::tcp_seq,
    pub snd_up: libc::tcp_seq,
    pub snd_wl1: libc::tcp_seq,
    pub snd_wl2: libc::tcp_seq,
    pub iss: libc::tcp_seq,
    pub irs: libc::tcp_seq,
    pub rcv_nxt: libc::tcp_seq,
    pub rcv_adv: libc::tcp_seq,
    pub rcv_wnd: u32,
    pub rcv_up: libc::tcp_seq,
    pub snd_wnd: u32,
    pub snd_cwnd: u32,
    pub snd_ssthresh: u32,
    pub t_maxopd: c_uint,
    pub t_rcvtime: u32,
    pub t_starttime: u32,
    pub t_rtttime: c_int,
    pub t_rtseq: libc::tcp_seq,
    pub t_rxtcur: c_int,
    pub t_maxseg: c_uint,
    pub t_srtt: c_int,
    pub t_rttvar: c_int,
    pub t_rxtshift: c_int,
    pub t_rttmin: c_uint,
    pub t_rttupdated: u32,
    pub max_sndwnd: u32,
    pub t_softerror: c_int,
    pub t_oobflags: c_char,
    pub t_iobc: c_char,
    pub snd_scale: c_uchar,
    pub rcv_scale: c_uchar,
    pub request_r_scale: c_uchar,
    pub requested_s_scale: c_uchar,
    pub ts_recent: u32,
    pub ts_recent_age: u32,
    pub last_ack_sent: libc::tcp_seq,
    pub cc_send: libc::tcp_cc,
    pub cc_recv: libc::tcp_cc,
    pub snd_recover: libc::tcp_seq,
    pub snd_cwnd_prev: u32,
    pub snd_ssthresh_prev: u32,
    pub t_badrxtwin: u32,
    pub xt_alignment_hack: libc::u_quad_t,
}

unsafe impl Encode for xtcpcb64 {
    const ENCODING: Encoding = Encoding::Struct(
        "xtcpcb64",
        &[
            <u32>::ENCODING,
            <libc::xinpcb64>::ENCODING,
            <u64>::ENCODING,
            <c_int>::ENCODING,
            <[c_int; 4]>::ENCODING,
            <c_int>::ENCODING,
            <c_uint>::ENCODING,
            <c_int>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <u32>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <c_uint>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <c_int>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <c_int>::ENCODING,
            <c_uint>::ENCODING,
            <c_int>::ENCODING,
            <c_int>::ENCODING,
            <c_int>::ENCODING,
            <c_uint>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <c_int>::ENCODING,
            <c_char>::ENCODING,
            <c_char>::ENCODING,
            <c_uchar>::ENCODING,
            <c_uchar>::ENCODING,
            <c_uchar>::ENCODING,
            <c_uchar>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <libc::tcp_cc>::ENCODING,
            <libc::tcp_cc>::ENCODING,
            <libc::tcp_seq>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <u32>::ENCODING,
            <libc::u_quad_t>::ENCODING,
        ],
    );
}

unsafe impl RefEncode for xtcpcb64 {
    const ENCODING_REF: Encoding = Encoding::Pointer(&Self::ENCODING);
}
