...
extern_class!(
    ...
    ///
    /// ### Important
    ///  To create a [`TVUserManager`](https://developer.apple.com/documentation/tvservices/tvusermanager) object, add the [`User Management Entitlement`](https://developer.apple.com/documentation/bundleresources/entitlements/com.apple.developer.user-management) capability to your app or app extension in Xcode, and select the Runs as Current User, Only When User-Independent Keychain is Available option. This enables the system to take care of separating each user’s data for your app.
    ///
    ///
    ///
    /// </div>
    /// To determine the current user’s profile, first check [`shouldStorePreferencesForCurrentUser`](https://developer.apple.com/documentation/tvservices/tvusermanager/shouldstorepreferencesforcurrentuser). If that value is [`false`](https://developer.apple.com/documentation/swift/false), display the profile picker to determine which profile to use for the current session, but don’t save the selected profile. If the value is [`true`](https://developer.apple.com/documentation/swift/true), and there isn’t a saved profile in [`NSUserDefaults`](https://developer.apple.com/documentation/foundation/userdefaults), display the profile picker and save the selected profile for future use. If the value is [`true`](https://developer.apple.com/documentation/swift/true) and there’s already a saved profile, skip the prompt and use the saved profile.
    ///
    ///
